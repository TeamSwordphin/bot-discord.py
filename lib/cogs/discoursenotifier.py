...
class Notifier(Cog):
	...
	async def post_update_log(self, ctx):
		if ctx.author.id == 105180779025334272:
			...
			if len(ctx.message.attachments) < 1:
				...
				await self.post_update(response_json["post_stream"]["posts"][0]["updated_at"], response_json)
	...
	async def on_ready(self):
		if not self.bot.ready:
			...
			self.bot.scheduler.add_job(self.check_post, CronTrigger(minute="3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,59"))
